...
const links = [
  { href: "/speech-demo", label: "SpeechToText" },
  { href: "#", label: "Contact" }
].map(link => {
        ...
        <div className="flex w-1/2 pl-4 text-sm">
            <ul className="list-reset flex justify-between flex-1 md:flex-none items-center">
                <Link href="/" className="mr-2">
                    <a  className="inline-block py-2 px-2 text-white no-underline hover:underline">Home</a>
                </Link>
                {links.map(({ key, href, label }) => (
                    <li key={key} className="mr-2">
                        <a className="inline-block text-gray-600 no-underline hover:text-gray-200 hover:underline py-2 px-2" href={href}>
                            {label}
                        </a>
                    </li>
                ))}
